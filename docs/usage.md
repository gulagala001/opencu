...
当前预发布版 1.1.0-rc.1 适配 DSH 0.1.7-alpha.1。旧宿主 DSH 0.1.6-alpha.2 请继续使用 1.0.4。固定版本安装命令及[验证范围与限制](release-1.1.0-rc.1.md)见项目首页。
...
连接日常 Chrome 并使用 **0.1.4 或更新的扩展**时，助手实际选择、创建的网页会自动归入浏览器顶部的原生标签组，显示 **OMD · 会话名称**，不同会话用颜色区分。网页从受控标签另开的子标签也会归组；仅列出标签、查看预览不会取得分组归属。会话改名时组名同步更新。

同一会话在每个 Chrome 窗口各有一个组，颜色保持一致，不为分组移动窗口。停止、断开控制或本轮结束后，保留的标签组显示 **（已停止）**；恢复控制时沿用仍存在的组。临时网页是否关闭继续遵循原有交付规则，分组本身不关闭网页，也不改动其他无关标签。

固定标签页不会被擅自取消固定；扩展会提示不能分组，网页操作仍可继续，取消固定后会自动归组。旧扩展保持原有操作能力：先在 **电脑 → 运行环境与权限** 更新浏览器连接，再到 `chrome://extensions` 重新加载 **Oh My DSH Computer Use**；如 Chrome 提示新增权限，请确认标签组权限。扩展工作进程恢复时会核对当前标签组并清除过期的控制中标记；整个浏览器重启后的分组恢复不作为持久会话记录。

此功能是**已连接 Chrome 的原生标签栏**，不改变 `@Browser` 内置浏览器的工作台标签栏。

### 平台与运行条件

| 部分 | 当前范围 |
| --- | --- |
| 通用插件 | Windows、macOS、Linux 共用 DSH 插件入口；复用宿主的模型、会话与工具配置。 |
...
「工作过程展示」沿用宿主的简洁、详细和完全展开三种模式；默认简洁。「详细」下执行中的操作组显示最新动作及状态，优先使用调用说明，否则显示文件路径、命令或搜索词。组内操作全部结束后，标题立即恢复操作汇总，不等待模型回复或整轮结束；已结束回合也始终显示汇总。

思考内容也跟随过程摘要收起；展开后可逐项查看。最终回复附带的思考统一显示在回复前，与前面的操作列表衔接，不散落在回复下面。这只调整已完成回复的展示，原始消息、思考文字和流式输出不变。

上下文注入、Todo／运行状态和回合内系统提示收在同一过程里，展开后仍可查看每条原记录；上下文压缩记录保持独立显示。会话初始系统提示与用户主动执行的命令保留独立入口，实际注入、记忆与压缩机制不受影响。
...
可以让助手“导出当前页面，保存页面图片和样式，并报告获取失败的素材”。当前支持 MHTML 页面快照，保留可读取的普通 HTML 输入、勾选与选项状态（含框架和开放 Shadow DOM），不复制密码、文件选择、隐藏字段及密码／一次性验证码自动填充字段；这是静态快照，不是脚本运行或浏览器会话备份。另支持已加载图片、字体、样式和视频资源的清单与保存；导出文件自动成为 DSH 持久附件，展开调用结果可打开，刷新和回合结束后仍保留。坏图和不可读取资源单独报告，懒加载内容需要先在页面加载。不提供 Google Workspace 专用格式、YouTube 字幕或完整流媒体导出；跨框架素材以实际发现和可读取的资源为准。

内嵌 SVG 可按清单 ID 保存为 `.svg` 文件，也包含在图片素材打包中；保存的是观察到的标记，外链资源与外部样式不会自动内联。

可以让助手检查当前网页的网络错误：读取连接后观察到的请求、重定向、状态码、请求头、请求体及完成的响应，不重新发送原请求。记录保留最近 200 条，正文返回限制为 1 MiB；超限、过期与未完成会明确报告。非 UTF-8 或二进制正文按 Base64 保留字节。该限制不等于底层读取或解压的内存上限。

助手可显示或隐藏当前会话的 DSH 浏览器预览，隐藏后网页继续运行；需要当前 DSH 页面实际确认，不会控制用户原生 Chrome 窗口的显示或最小化。临时页仍需保留为交付页，才能跨回合继续使用。
...
内置浏览器使用 macOS 的凭据存储保存登录信息。首次启动、浏览器版本或签名变化时，系统可能询问是否允许 Chrome 访问 Safe Storage。这个提示来自 macOS，不是模型索取密码；不确定时可以拒绝，部分已有登录状态可能无法解密。连接日常 Chrome 时，请保持原浏览器的配置与钥匙串设置。
