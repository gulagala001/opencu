...
for(const backend of ['managed','extension'])test(backend+': nested frame annotations follow real pixels, clipping, stacking and frame-specific styles',{timeout:30000},async t=>{
  ...
  // Serve both loopback families so localhost can use IPv6 while the main
  // page uses IPv4. The two hostnames still preserve cross-site/OOPIF coverage.
  const crossServer=createServer(server.listeners('request')[0]);
  await new Promise((resolve,reject)=>{crossServer.once('error',error=>{server.closeAllConnections();server.close();reject(error);});crossServer.listen({port:server.address().port,host:'::1',ipv6Only:true},resolve);});
