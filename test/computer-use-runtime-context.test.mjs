...
test('server restart announces lost CU bindings once without rearranging existing context',()=>{
  ...
  const manager={sessions:new Map()};assert.equal(announceFreshComputerRuntime(manager,session),true);assert.equal(events[0],system);assert.equal(events[1],call);assert.equal(events[2].data.source.kind,'plugin:trisoul-x:computer-use-runtime');assert.match(events[2].data.content[0].text,/Do not reuse old element IDs or resend/);
