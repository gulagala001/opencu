...
A closed group's header names the first three categories from its ranked summary, with a separate operation count and failure/stopped badges. A group without categories uses the thinking label. A running header names its live tool category, otherwise thinking; Standard appends live detail. Live titles remain visible for at least 150ms, retaining only the newest pending title.
...
Adjacent reasoning blocks in one Assistant share a disclosure while preserving their full text. Individual reasoning starts collapsed, including while streaming. All modes preview the latest paragraph whose first line ends with a newline; an unfinished single line previews its first nonempty line. Later text in that paragraph does not change the preview. After settlement, the mode table applies. Expanded reasoning uses compact Markdown typography.
...
OpenCU keeps inner disclosure choices when a parent is folded and preserves an open process at completion. Its bounded process drawer uses a 2px gap and `min(280px, 45vh)` height cap; Computer Use activities have their own category.
