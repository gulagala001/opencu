...
function ChromeSetup({ extension, busy, act, onError }) {
  ...
  return <>
    ...
    {installation?.supported ? <>
      {(!installation.prepared || installation.updateAvailable) && <div className="tx-cu-setup-install"><p>先准备本机连接程序，再在 Chrome 中加载扩展。开发版无需商店账号，首次加载需要开启 Chrome 的开发者模式。{installation.platform==='win32'&&'Windows 首次准备需要 .NET 10 SDK，用于编译本机连接程序。'}</p><button type="button" disabled={!!busy || installation.preparing} onClick={() => act('install-extension')}>{busy === 'install-extension' || installation.preparing ? '正在准备…' : installation.updateAvailable ? '更新 Chrome 连接' : '准备 Chrome 连接'}</button></div>}
