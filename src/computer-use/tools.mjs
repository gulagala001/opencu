...
export const COMPUTER_USE_GUIDE = [
  ...
  "Use methods on the returned target: await tab.setValue(42,\"text\"); await tab.click(51); await tab.getAXState(). Native targets use app.setValue, app.click and app.getAXState. The cua object discovers/selects targets and provides documentation; it does not perform target actions. First-use documentation gives the methods and limitations of the selected backend; reread an unfamiliar topic with nodeRepl.write(await cua.documentation(topic)) using core, browser, app, recovery, files, screenshots, webmcp or network; do not invent methods on cua or assume every backend has every optional capability.",
  ...
  "Plan each call around the next decision: complete the known fields or other deterministic steps together, then inspect the result before acting on newly revealed UI. Preserve work already done when recovering from an error. An ordinary failed call does not imply lost bindings; reuse initialized targets and recover at the failed step instead of replaying the whole batch or resetting immediately.",
  "Finish when the requested result has a reliable visible confirmation and no conflicting evidence. If an action had no effect, inspect the current blocker before repeating it. Keep the requested browser/profile and any unsaved page state; reselecting a target must not become an unnecessary navigation. Explain a real interruption or missing capability clearly, with the completed and remaining work, rather than treating an attempted action as success.",
