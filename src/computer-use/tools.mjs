...
export const COMPUTER_USE_GUIDE = [
  ...
  "Use methods on the returned target: await tab.setValue(42,\"text\"); await tab.click(51); await tab.getAXState(). Native targets use app.setValue, app.click and app.getAXState. The cua object discovers/selects targets and provides documentation; it does not perform target actions. First-use documentation gives the methods and limitations of the selected backend; reread an unfamiliar topic with nodeRepl.write(await cua.documentation(topic)) using core, browser, app, recovery, files, screenshots or webmcp; do not invent methods on cua or assume every backend has every optional capability.",
